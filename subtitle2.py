...
text="""after the events of the
last part where i
accidentally leaked
my base coordinates
i do the first thing that
comes to mind...
grief my own base
nah im just kidding
yeah i wasnt kidding
after i go back to the
enemy's headquarters
and i do this really cool
thing called fighting and
stealing
we only do a tad and when
i say a tad i mean a lot of
fighting and stealing
around here
of course they werent so
happy about that...
...
with open('TimelineBase.xml', mode='r') as f:
    timelinebase=f.read()
vpos = timelinebase.find('<video>')+8
with open('ClipItemBase.xml', mode='r') as f:
    clipitembase=f.read()
...
for text in texts:
    ...
    clipstart='45'
    clipend='60'    
    clip=clipitembase.replace('_filename_',name)
    clip=clip.replace('_path_',path)
    clip=clip.replace('_start_',clipstart)
    clip=clip.replace('_end_',clipend)
    clips=clips+clip


subtitle_timeline = timelinebase[:vpos]+clips+timelinebase[vpos:]
with open('subtitle_time.xml', mode='w') as f:
    f.write(subtitle_timeline)